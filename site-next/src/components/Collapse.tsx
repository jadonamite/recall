 ...
 *
 * The section owns four viewports of scroll. For all of it the viewport is held
...
export default function Collapse({ data }: { data: Consumer }) {
  ...
  const wallScale = useTransform(scrollYProgress, [0.10, 0.50], [1, 0.34]);
  const wallOpacity = useTransform(scrollYProgress, [0.12, 0.46], [1, 0]);
  const wallBlur = useTransform(scrollYProgress, [0.12, 0.46], [0, 14]);
  const wallFilter = useTransform(wallBlur, (b) => `blur(${b}px)`);

  const fixOpacity = useTransform(scrollYProgress, [0.32, 0.52], [0, 1]);
  const fixY = useTransform(scrollYProgress, [0.32, 0.58], [56, 0]);
...
function Readout({
  ...
  useMotionValueEvent(progress, "change", (v) => setFlipped(v >= 0.36));
